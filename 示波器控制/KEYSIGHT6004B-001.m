classdef KEYSIGHT6004B_001 < Device
    ...
    methods
        ...
        function AutoCenterDisplay(obj,chID,time,~)
            ...
            if flagAutoSetVerticalScale
                ...
                fprintf(g,cmd);
                %调到4
                cmd = sprintf(':CHANnel%d:SCALe %f V',chID,0.004);
                fprintf(g,cmd);
                cmd = sprintf(':MEASure:VAVerage? CHANnel%d',chID);
                Vavg = str2double(query(g,cmd));
                cmd = sprintf(':CHANnel%d:OFFSet %f V',chID,Vavg);
                fprintf(g,cmd);
                %调到3
                cmd = sprintf(':CHANnel%d:SCALe %f V',chID,0.003);
                fprintf(g,cmd);
                cmd = sprintf(':MEASure:VAVerage? CHANnel%d',chID);
                Vavg = str2double(query(g,cmd));
                cmd = sprintf(':CHANnel%d:OFFSet %f V',chID,Vavg);
                fprintf(g,cmd);
                %调到2
                cmd = sprintf(':CHANnel%d:SCALe %f V',chID,0.002);
                fprintf(g,cmd);
                cmd = sprintf(':MEASure:VAVerage? CHANnel%d',chID);
                Vavg = str2double(query(g,cmd));
                cmd = sprintf(':CHANnel%d:OFFSet %f V',chID,Vavg);
